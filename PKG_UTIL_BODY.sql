CREATE OR REPLACE PACKAGE BODY PKG_UTIL IS
   ...
   FUNCTION normalize(str1 IN VARCHAR2) RETURN VARCHAR2 IS
    pos           INT;
    str           VARCHAR2(3000);
    chars_special VARCHAR2(255) := chr( 192 ) ||chr( 198 ) ||chr( 223 ) ||chr( 193 ) ||chr( 194 ) ||chr( 195 ) ||chr( 196 ) ||chr( 197 ) ||
    ...
    -- chars_special VARCHAR2(255) := 'ÀÆßÁÂÃÄÅÈÉÊËÌÍÎÏÐÑÒÓÔÕÖØÙÚÛÜÝY';
    chars_normal  VARCHAR2(255) := 'AABAAAAAEEEEIIIIIDNOOOOOOUUUUYY';
  ...
  FUNCTION foneticabr(str1 IN VARCHAR2) RETURN VARCHAR2 IS   
    chars_s VARCHAR2(255) := 'BL,BR,Ç,CHR,CA,SCE,SCI,CE,CH,CI,CK,CO,CS,CT,CU,C,GE,GI,GL,GM,GR,LH,LT,L,MD,MG,MJ,NG,NH,NJ,N,PH,PR,QUE,QUI,Q,RG,RJ,RM,RS,RT,SM,ST,TH,TL,TR,TS,W,X,Y,Z';
    chars_r VARCHAR2(255) := 'B,B,S,KR,K,SE,SI,S,S,S,K,K,S,T,K,K,J,J,G,M,G,L,T,R,M,G,J,G,N,J,M,F,P,KE,KI,K,G,J,M,S,T,M,T,T,T,T,S,U,S,I,S';
    str     VARCHAR2(3000);
  ...
  BEGIN
    ...
    FOR pos IN 1 .. regexp_count(chars_s, ',') + 1 LOOP
